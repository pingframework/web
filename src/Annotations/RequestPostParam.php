...
class RequestPostParam implements Injector
{
    ...
    public function inject(
        ...
        $payload = (array)$runtime[ServerRequestInterface::class]->getParsedBody();

        $name = $this->name ?? Strings::camelCaseToUnderscore($rp->getName());

        if (!Arrays::isExists($payload, $name)) {
            if ($rp->allowsNull()) {
                return null;
            }

            throw new RuntimeException("Request post param '{$name}' is not exists.");
        }
